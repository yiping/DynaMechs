...
for i=1:2
   ...
   for j=1:20000
      [next,buffer] = strtok(buffer,';');  %  parse next column label, strtok returns the 1st token delimited by ';'
      next = strtrim(next);  %  remove insignificant whitespace
...
labels = blanks(maxlen); % Create string of blank characters
...
for i=1:2
   ...
   for j=1:ncols
      ...
      labels(n,1:length(next)) = next;    %  append to the labels matrix - Wow!
   end
end
diplayLabels = labels(1:ncols,:);
names        = labels((ncols+1):end,:);  %  matlab names
...
%  names(i, :) means the ith row of matlab lables matrix
for i=1:ncols
    tok = regexp(names(i,:),'(.*)\((.*?)\)','tokens'); %  returns tokenstring
    if length(tok) > 0
       name = strcat(tok{1}(1),'(',tok{1}(2),',:)'); % if a group, combine name and index
    else
       name = names(i,:); 
    end
    commands{i} = strcat(name,'=data(:,',num2str(i),')'';'); % compose a matlab command
end

data = fscanf(fin,'%f');  %  Load the numerical values into one long vector, ROW major
%size(data)
