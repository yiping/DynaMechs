...
conjunction with a dmSystem object to perform the dynamic simulation.
After construction of a derived class, one or more dmSystem objects must
be added to this object with a call to the \b addSystem function.  Pointers
to currently assigned systems can be obtained with a call to the \b
...
removed from an integrator with calls to one of the \b removeSystem
...
to the user to manage the dmSystem s (e.g., a pointer to a system could be
...
integrate anything and a call to \b isReadyToSim returns false.  After all
...
initialized, this function will return true.  A call to \b synchronizeState
...
to dmLink::setState.  In this event, these calls must be followed by a
call to \b synchronizeState  to update the integrator with the new state of
the system.

The system is simulated by calling the \b simulate function which is a pure
virtual function implemented in the derived integrator classes (see
dmIntegEuler, dmIntegRK4, dmIntegRK45, etc.).  The \b idt parameter,
