...
: wxFrame(NULL, -1, title, pos, size) {
	...
	// Create panels
	{
		...
		// robot controller related display
...
{
	wxString dir = wxDirSelector(wxT("Select the Data Save Directory"),wxString(huboCtrl->dataSaveDirectory.c_str(),wxConvUTF8));
	dir += wxT("/");
	huboCtrl->dataSaveDirectory = dir.mb_str();
	cout << "Data Directory Changed to " << huboCtrl->dataSaveDirectory << endl;
}
