function hop()
    ...
    for i=1:length(VX)
        ...
        ssParams = [params(1) 0 params(2) params(2)];
        ssInit = [params(3) vx params(4)]';
        VXparams{i} = ssParams;
        VXinits{i} = ssInit;
        
        stateInit(3:5) = ssInit;
        [Js_ss Ju_ss] = evalJacobian(stateInit, ssParams, dynParams);
        K_ss = - Ju_ss\Js_ss;
        VXfb{i} = K_ss;
    ...
    for i=16:numRules
        ...
        tStanceDes = 10^-0.2*vxf^-0.82; % .35s estimated from paper
        ...
        options = optimset('display','iter','TolFun',1e-9,'TolX',1e-12,'MaxFunEvals',1000);
...
%         [params resnorm res] = lsqnonlin(@periodResidual,params0,[0,-pi/4,1000/1e4,1000/1e4],[pi/4,pi/4,150000/1e4,150000/1e4],options);
         ...
         [Js Ju] = evalJacobian(stateInit, params, dynParams);
         dParams = - (Ju\res)'
