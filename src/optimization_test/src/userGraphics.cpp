...
void BasicGLPane::userGraphics()
{
	...
	{
		Vector3F p_zmp = humanoidCtrl->getZmpPos();
		Vector3F p_zmp_opt = humanoidCtrl->getZmpPosOpt();
		glPushMatrix();
	...
	if (frame->enableExtForcesCheckBox->IsChecked())
	{
		...
		Vector3F f1 = Vector3F::Zero();
		f1 = R1.transpose()* (simThread->fExtTorso.tail(3));
		f1 = f1/500;
